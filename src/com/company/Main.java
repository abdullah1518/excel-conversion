...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.length ; i++){
            ...
            String listout = list[i];

            lost.add(listout);

            sb.append(lost.get(i));

        }
        System.out.println(sb);


        //separating values and cleaning
        String data2 = sb.toString();
        ArrayList<String> lost2 = new ArrayList<String>();
        String[] list2 = data2.split("\t");
        StringBuilder sb2 = new StringBuilder();

        for (int k = 0; k < list2.length ; k++){

            String listout2 = list2[k];

            //adding list values to array values
            lost2.add(listout2);
            ...
            if((k)%9 == 0 && k>0){
                ...
                sb2.append(lost2.get(k));
            }
        }
        System.out.println(sb2);
        //Trying to clean name separators
        StringBuilder sb3 = new StringBuilder();
        for (int x =1; x < sb2.length(); x++){
            String charcheck = sb2.substring(x-1, x);

            if (charcheck.equals("*") || charcheck.equals("|") || charcheck.equals("#")) {

            }
            else{
                sb3.append(charcheck);
            }
        ...
        for (int x =3; x < sb3.length(); x++){
            String charcheck = sb3.substring(x-3, x-2);
            String charcheck2 = sb3.substring(x-2, x-1);
            String charcheck3 = sb3.substring(x-1, x);
            if (!((charcheck2.chars().allMatch( Character::isDigit )) || charcheck2.equals("$") ) && (charcheck.equals(" "))) {
                sb4.append("'");
                sb4.append(charcheck2);
            }
            if (!((charcheck2.chars().allMatch( Character::isDigit )) || charcheck2.equals("$") ) && (charcheck3.equals(" "))) {
                sb4.append(charcheck2);
                sb4.append("'");
            }
            else{
